...
# 第一步：在远程服务器上备份上传的图片
echo "正在备份上传的图片..."
ssh -o StrictHostKeyChecking=no $REMOTE_USER@$REMOTE_HOST "DEPLOY_PATH='$DEPLOY_PATH'; BACKUP_DIR=\"/tmp/uploads_backup_\$(date +%s)\"; if [ -d \"\$DEPLOY_PATH/public/uploads\" ]; then mkdir -p \"\$BACKUP_DIR\" && cp -r \"\$DEPLOY_PATH/public/uploads\"/* \"\$BACKUP_DIR/\" 2>/dev/null || true && echo \"图片备份成功，备份位置: \$BACKUP_DIR\"; else echo \"没有需要备份的图片\"; fi"
...
# 第三步：传输代码到远程服务器
echo "正在传输代码到远程服务器..."
scp backend_temp.tar.gz $REMOTE_USER@$REMOTE_HOST:/tmp/
